...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/raphaelgabiazon/Desktop/PsychoPy-fMRI-Task-Pilot-01-Builder/fMRI_MemoryTask_Builder.py',
...
# Function to create txt files per run
def save_experiment_data(expInfo, block_num, 
                         Encoding_Data_Face, Encoding_Data_Place, Encoding_Data_Pair,
                         Recog_Data_Face, Recog_Data_Place, Recog_Data_Pair,
                         Encoding_Response_Data_Face, Encoding_Response_Data_Place, Encoding_Response_Data_Pair,
                         Recog_Response_Data_Face, Recog_Response_Data_Place, Recog_Response_Data_Pair):
    ...
    save_three_column_file(Encoding_Data_Face, os.path.join(output_directory, f'encoding_data_face_run{block_num}_P{participant_number}.txt'))
    save_three_column_file(Encoding_Data_Place, os.path.join(output_directory, f'encoding_data_place_run{block_num}_P{participant_number}.txt'))
    save_three_column_file(Encoding_Data_Pair, os.path.join(output_directory, f'encoding_data_pair_run{block_num}_P{participant_number}.txt'))

    save_three_column_file(Recog_Data_Face, os.path.join(output_directory, f'recog_data_face_block{block_num}_P{participant_number}.txt'))
    save_three_column_file(Recog_Data_Place, os.path.join(output_directory, f'recog_data_place_block{block_num}_P{participant_number}.txt'))
    save_three_column_file(Recog_Data_Pair, os.path.join(output_directory, f'recog_data_pair_block{block_num}_P{participant_number}.txt'))

    # Convert the remaining encoding and recognition response data in the dictionaries to lists and save them to text files
    save_three_column_file(list(Encoding_Response_Data_Face.values()), os.path.join(output_directory, f'encoding_response_data_face_run{block_num}_P{participant_number}.txt'))
    save_three_column_file(list(Encoding_Response_Data_Place.values()), os.path.join(output_directory, f'encoding_response_data_place_run{block_num}_P{participant_number}.txt'))
    save_three_column_file(list(Encoding_Response_Data_Pair.values()), os.path.join(output_directory, f'encoding_response_data_pair_run{block_num}_P{participant_number}.txt'))

    # Save the recognition data in the lists to text files
    save_three_column_file(Recog_Response_Data_Face, os.path.join(output_directory, f'recog_response_data_face_run{block_num}_P{participant_number}.txt'))
    save_three_column_file(Recog_Response_Data_Place, os.path.join(output_directory, f'recog_response_data_place_run{block_num}_P{participant_number}.txt'))
    save_three_column_file(Recog_Response_Data_Pair, os.path.join(output_directory, f'recog_response_data_pair_run{block_num}_P{participant_number}.txt'))


# Generate 11 numbers between 5 and 7 that sum up to 66.0 
jitter_duration = generate_jitter_time(min_=5, max_=7, count=12, target=72.0)

# Initialize variables
volume_counter = 1
volume_total = 708 # (10.0 sec + 72.0 sec + 36.0 sec) x 6 conditions
...
# Initalize instruction start time, SOA after block 1 depend on this
next_Instruction_started_time = 0  
...
# Lists to store stimuli show for 'esc' no response data

# BLOCK 1 RUN 1
recog_face_displayed_images_block_1_run_1 = [] 
recog_place_displayed_images_block_1_run_1 = [] 
recog_pair_displayed_images_block_1_run_1 = [] 

# BLOCK 2 RUN 2
recog_face_displayed_images_block_2_run_2 = [] 
recog_place_displayed_images_block_2_run_2 = [] 
recog_pair_displayed_images_block_2_run_2 = [] 

# BLOCK 3 RUN 3
recog_face_displayed_images_block_3_run_3 = [] 
recog_place_displayed_images_block_3_run_3 = [] 
recog_pair_displayed_images_block_3_run_3 = [] 
...
image_counter = 0 # Initalize image counter which indicates that first image is shown
...
# Function to remove old encoding response list not shown during recognition
def handle_esc_experiment_data(
    recog_key, block_num, run_num, recog_face_stims, 
    recog_place_stims,recog_pair_stims, displayed_images_face, 
    displayed_images_place, displayed_images_pair, encoding_response_data_face, 
    encoding_response_data_place, encoding_response_data_pair):
    
    displayed_images = []
    
    if recog_key == f'Face_Recog_Key_Block_{block_num}_Run_{run_num}':
        displayed_images = [img.replace('_Recog', '_Encoding') for img in displayed_images_face]
        recog_face_stims = [recog_face_image.replace('_Recog', '_Encoding') for recog_face_image in recog_face_stims]
        for image in list(encoding_response_data_face.keys()):
            # If an image was not shown during recognition, remove it from the encoding response data
            if image not in displayed_images and image in recog_face_stims:
                encoding_response_data_face.pop(image, None)

    elif recog_key == f'Place_Recog_Key_Block_{block_num}_Run_{run_num}':
        displayed_images = [img.replace('_Recog', '_Encoding') for img in displayed_images_place]
        recog_place_stims = [recog_place_image.replace('_Recog', '_Encoding') for recog_place_image in recog_place_stims]
        for image in list(encoding_response_data_place.keys()):
            # If an image was not shown during recognition, remove it from the encoding response data
            if image not in displayed_images and image in recog_place_stims:
                encoding_response_data_place.pop(image, None)

    elif recog_key == f'Pair_Recog_Key_Block_{block_num}_Run_{run_num}':
        displayed_images = [img.replace('_Recog', '_Encoding') for img in displayed_images_pair]
        recog_pair_stims = [recog_pair_image.replace('_Recog', '_Encoding') for recog_pair_image in recog_pair_stims]
        for image in list(encoding_response_data_pair.keys()):
            # If an image was not shown during recognition, remove it from the encoding response data
            if image not in displayed_images and image in recog_pair_stims:
                encoding_response_data_pair.pop(image, None)
...
encodeTrialRun2_Image = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-1.0)
encodeFixationText_Run2 = visual.TextStim(win=win, name='encodeFixationText_Run2',
    text='',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
recogTrialRun2_Image = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-1.0)
recogFixationText_Run2 = visual.TextStim(win=win, name='recogFixationText_Run2',
    text='',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
encodeTrialRun3_Image = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-1.0)
encodeFixationText_Run3 = visual.TextStim(win=win, name='encodeFixationText_Run3',
    text='',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
recogTrialRun3_Image = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-1.0)
recogFixationText_Run3 = visual.TextStim(win=win, name='recogFixationText_Run3',
    text='',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
for thisBlock1Loop in Block1Loop:
    ...
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        # check for quit (typically the Esc key)
        if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
            # Update InstructionsText at encoding block 1 run 1 upon 'esc'
            save_experiment_data(
                expInfo, 
                block_num=Block_1_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_1_Run_1, 
                Encoding_Data_Place=Encoding_Data_Place_Block_1_Run_1,
                Encoding_Data_Pair=Encoding_Data_Pair_Block_1_Run_1, 
                Recog_Data_Face=Recog_Data_Face_Block_1_Run_1, 
                Recog_Data_Place=Recog_Data_Place_Block_1_Run_1, 
                Recog_Data_Pair=Recog_Data_Pair_Block_1_Run_1,
                Encoding_Response_Data_Face=Encoding_Response_Data_Face_Block_1_Run_1, 
                Encoding_Response_Data_Place=Encoding_Response_Data_Place_Block_1_Run_1, 
                Encoding_Response_Data_Pair=Encoding_Response_Data_Pair_Block_1_Run_1,
                Recog_Response_Data_Face=Recog_Response_Data_Face_Block_1_Run_1, 
                Recog_Response_Data_Place=Recog_Response_Data_Place_Block_1_Run_1, 
                Recog_Response_Data_Pair=Recog_Response_Data_Pair_Block_1_Run_1
            )
            core.quit()
    ...
    for thisEncodeLoop_Block1Run1 in encodeLoop_Block1Run1:
        ...
        # Call set_ISI_time_and_message to display fixation cross and jitter time
        ISI_Time, ISI_Message = set_ISI_time_and_message(Fix_Cross_Started, jitter_duration, current_jitter_index)
        
        # Add jitter time to data
        thisExp.addData('jitter_time', ISI_Time)
        
        
        current_time = timer.getTime() - start_time 
        
        if image_counter == 1: # If not the first instruction screen
            thisExp.addData('start_time',current_time) # Add start time to data
        ...
        while continueRoutine and routineTimer.getTime() < 3.0 + ISI_Time: # Replaced 10002 with 3.0 + ISI_Time for Block 1 Run 1 Encoding
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if encodeFixationText_Run1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > encodeFixationText_Run1.tStartRefresh + ISI_Time-frameTolerance: # Replaced 9999.0 with ISI_Time for Block 1 Run 1 Encoding
                    # keep track of stop time/frame for later
            ...
            if encodeFixationText_Run1.status == STARTED:  # only update if drawing
                encodeFixationText_Run1.setText(ISI_Message, log=False)
            ...
            # check for quit (typically the Esc key)
            if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
                # Update encodeTrial_Run1 at encoding block 1 run 1 upon 'esc'
                save_experiment_data(
                expInfo, 
                block_num=Block_1_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_1_Run_1, 
        ...
        if current_encode_index == 12: # Last stimulus is shown
            ...
            image_counter = 0 # Reset image_counter
        ...
        if routineForceEnded:
            ...
            routineTimer.addTime(-(3.0 + ISI_Time)) # Replaced -10002 with 3.0 + ISI_Time for Block 1 Run 1 Encoding
        thisExp.nextEntry()
    ...
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        # check for quit (typically the Esc key)
        if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
            # Update InstructionsText at recognition block 1 run 1 upon 'esc'
            save_experiment_data(
                expInfo, 
                block_num=Block_1_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_1_Run_1, 
    ...
    for thisRecognitionLoop_Block1Run1 in recognitionLoop_Block1Run1:
        ...
        while continueRoutine and routineTimer.getTime() < 3.0 + ISI_Time: # Replaced 10002 with 3.0 + ISI_Time for Block 1 Run 1 Recognition
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if recogTrialRun1_Image.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
                ...
                if Recog_Key_Block_1_Run_1 == 'Face_Recog_Key_Block_1_Run_1':
                    ...
                    recog_face_displayed_images_block_1_run_1.append(current_displayed_image) 
                    ...
                    recog_place_displayed_images_block_1_run_1.append(current_displayed_image) 
                    ...
                    recog_pair_displayed_images_block_1_run_1.append(current_displayed_image) 
            ...
            if recogFixationText_Run1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > recogFixationText_Run1.tStartRefresh + ISI_Time-frameTolerance: # Replaced 9999.0 with ISI_Time for Block 1 Run 1 Recognition
                    # keep track of stop time/frame for later
            ...
            if recogFixationText_Run1.status == STARTED:  # only update if drawing
                recogFixationText_Run1.setText(ISI_Message, log=False)
            ...
            if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
                ...
                # Edit lists for txt at recognitionTrial_Run1 recognition block 1 run 1 upon 'esc'
                handle_esc_experiment_data( 
                recog_key=Recog_Key_Block_1_Run_1,
                block_num=1,
                run_num=Block_1_Run,
                recog_face_stims = Face_Recog_Images_Block_1_Run_1,
                recog_place_stims = Place_Recog_Images_Block_1_Run_1,
                recog_pair_stims= Pair_Recog_Images_Block_1_Run_1,
                displayed_images_face=recog_face_displayed_images_block_1_run_1,
                displayed_images_place=recog_place_displayed_images_block_1_run_1,
                displayed_images_pair=recog_pair_displayed_images_block_1_run_1,
                encoding_response_data_face=Encoding_Response_Data_Face_Block_1_Run_1,
                encoding_response_data_place=Encoding_Response_Data_Place_Block_1_Run_1,
                encoding_response_data_pair=Encoding_Response_Data_Pair_Block_1_Run_1
            )
                
                # Update recognitionTrial_Run1 at recognition  block 1 run 1 upon 'esc'
                save_experiment_data(
                expInfo, 
                block_num=Block_1_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_1_Run_1, 
        ...
        if routineForceEnded:
            ...
            routineTimer.addTime(-(3.0 + ISI_Time)) # Replaced -10002 with 3.0 + ISI_Time for Block 1 Run 1 Recognition
        thisExp.nextEntry()
...
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
frameN = -1
Time_Since_Run.reset()  # Reset clock for run 2 onset and stop times
next_Instruction_started_time = 0  # Initialize instruction start time, run 2 onset times depend on this
image_counter = 0 # or instuction onset times i.e., 1st image means Instruction time based on Time_Since_Run 
...
for thisBlock2Loop in Block2Loop:
    ...
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        if InstructionText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            win.timeOnFlip(InstructionText, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            Instruction_start = next_Instruction_started_time if next_Instruction_started_time != 0 else Time_Since_Run.getTime()
            thisExp.addData('InstructionText.started', Instruction_start)  # InstructionText.started for Block 2 Run 2 Encoding
            InstructionText.setAutoDraw(True)
        if InstructionText.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > InstructionText.tStartRefresh + 10-frameTolerance:
                ...
                InstructionText.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                Instruction_stop = Time_Since_Run.getTime()
                thisExp.addData('InstructionText.stopped', Instruction_stop)  # Instruction_stop for Block 2 Run 2 Encoding
                InstructionText.setAutoDraw(False)
            else: # If Instruction_stop not updated
                Instruction_stop = Time_Since_Run.getTime()
                thisExp.addData('InstructionText.stopped', Instruction_stop) # Instruction_stop for Block 2 Run 2 Encoding
        ...
        if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
            ...
            # Update InstructionsText at encoding block 2 run 2 upon 'esc'
            save_experiment_data(
                expInfo, 
                block_num=Block_2_Run, 
                ...
                Recog_Data_Face=Recog_Data_Face_Block_2_Run_2, 
                Recog_Data_Place=Recog_Data_Place_Block_2_Run_2, 
                Recog_Data_Pair=Recog_Data_Pair_Block_2_Run_2,
                ...
                Recog_Response_Data_Face=Recog_Response_Data_Face_Block_2_Run_2, 
                Recog_Response_Data_Place=Recog_Response_Data_Place_Block_2_Run_2, 
                Recog_Response_Data_Pair=Recog_Response_Data_Pair_Block_2_Run_2
            )
    ...
    for thisEncodeLoop_Block2Run2 in encodeLoop_Block2Run2:
        ...
        # keep track of which components have finished
        encodeTrial_Run2Components = [encodeTrialRun2_Image, encodeFixationText_Run2, encodeTrialTestKeyRespRun2]
        ...
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        frameN = -1
        next_Instruction_started_time = 0 # Reset following every condition - Block 2 Run 2 Encoding Remove?
        
        # --- Run Routine "encodeTrial_Run2" ---
        while continueRoutine and routineTimer.getTime() < 3.0 + ISI_Time: # Replaced 10002 with 3.0 + ISI_Time for Block 2 Run 2 Encoding
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if encodeTrialRun2_Image.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                ...
                # add timestamp to datafile
                encodeImageRun2_started_time = Instruction_stop if image_counter == 0 else fixation_stopped_time 
                thisExp.addData('encodeTrialRun2_Image.started', encodeImageRun2_started_time)  # encodeTrialRun2_Image for Block 2 Run 2 Encoding
            ...
            if encodeTrialRun2_Image.status == STARTED:
                ...
                if tThisFlipGlobal > encodeTrialRun2_Image.tStartRefresh + 3-frameTolerance:
                    ...
                    thisExp.addData('encodeTrialRun2_Image.stopped', encodeImageRun2_stopped_time)  # encodeTrialRun2_Image.stopped for Block 2 Run 2 Encoding
            ...
            # *encodeFixationText_Run2* updates
            if encodeFixationText_Run2.status == NOT_STARTED and tThisFlip >= 3.0-frameTolerance:
                # keep track of start time/frame for later
                encodeFixationText_Run2.frameNStart = frameN  # exact frame index
                encodeFixationText_Run2.tStart = t  # local t and not account for scr refresh
                encodeFixationText_Run2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(encodeFixationText_Run2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                Fixation_Started_Time = encodeImageRun2_stopped_time # Fixation onset depends on Image offset
                thisExp.addData('encodeFixationText_Run2.started', Fixation_Started_Time) # encodeFixationText_Run2.started for Block 2 Run 2 Encoding
                encodeFixationText_Run2.setAutoDraw(True)
            if encodeFixationText_Run2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > encodeFixationText_Run2.tStartRefresh + ISI_Time-frameTolerance: # Replaced 9999.0 with ISI_Time for Block 2 Run 2 Encoding
                    # keep track of stop time/frame for later
                    encodeFixationText_Run2.tStop = t  # not accounting for scr refresh
                    encodeFixationText_Run2.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    fixation_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('encodeFixationText_Run2.stopped', fixation_stopped_time)  # encodeFixationText_Run2.stopped for Block 2 Run 2 Encoding
                    next_Instruction_started_time = fixation_stopped_time  # Set onset of next instruction time
                    encodeFixationText_Run2.setAutoDraw(False)
                else: # If fixation_stopped_time not updated
                    fixation_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('encodeFixationText_Run2.stopped', fixation_stopped_time)  # encodeFixationText_Run2.stopped for Block 2 Run 2 Encoding
                    next_Instruction_started_time = fixation_stopped_time  # Set onset of next instruction time
            ...
            if encodeFixationText_Run2.status == STARTED:  # only update if drawing
                encodeFixationText_Run2.setText(ISI_Message, log=False)
            ...
            if encodeTrialTestKeyRespRun2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > encodeTrialTestKeyRespRun2.tStartRefresh + 6-frameTolerance:
                    # keep track of stop time/frame for later
                    ...
                    # add timestamp to datafile
                    encodeTrialTestKeyRespRun2_stopped_time = Time_Since_Run.getTime()
            ...
            if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
                ...
                # Update encodeTrial_Run2 at encoding block 2 run 2 upon 'esc'
                save_experiment_data(
                expInfo, 
                block_num=Block_2_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_2_Run_2, 
        ...
        if routineForceEnded:
            ...
            routineTimer.addTime(-(3.0 + ISI_Time)) # Replaced -10002 with 3.0 + ISI_Time for Block 2 Run 2 Encoding
        thisExp.nextEntry()
    ...
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        if InstructionText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            Instruction_start = next_Instruction_started_time if next_Instruction_started_time != 0 else Time_Since_Run.getTime()
            thisExp.addData('InstructionText.started', Instruction_start)  # InstructionText.started for Block 2 Run 2 Recognition
            InstructionText.setAutoDraw(True)
        if InstructionText.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > InstructionText.tStartRefresh + 10-frameTolerance:
                ...
                # add timestamp to datafile
                Instruction_stop = Time_Since_Run.getTime()
                thisExp.addData('InstructionText.stopped', Instruction_stop)  # InstructionText.stopped for Block 2 Run 2 Recognition
                InstructionText.setAutoDraw(False)
            else: # If Instruction_stop not updated
                Instruction_stop = Time_Since_Run.getTime()
                thisExp.addData('InstructionText.stopped', Instruction_stop)  # InstructionText.stopped for Block 2 Run 2 Recognition
        ...
        if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
            ...
            # Update InstructionsText at recognition block 2 run 2 upon 'esc'
            save_experiment_data(
                expInfo, 
                block_num=Block_2_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_2_Run_2, 
    ...
    for thisRecognitionLoop_Block2Run2 in recognitionLoop_Block2Run2:
        ...
        # keep track of which components have finished
        recognitionTrial_Run2Components = [recogTrialRun2_Image, recogFixationText_Run2, recogTrialTestKeyRespRun2]
        ...
        while continueRoutine and routineTimer.getTime() < 3.0 + ISI_Time: # Replaced 10002 with 3.0 + ISI_Time for Block 2 Run 2 Recognition
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if recogTrialRun2_Image.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                ...
                # add timestamp to datafile
                recognitionImageRun2_started_time = Instruction_stop if image_counter == 0 else fixation_stopped_time 
                thisExp.addData('recogTrialRun2_Image.started', recognitionImageRun2_started_time)  # recogTrialRun2_Image.started for Block 2 Run 2 Recognition
            ...
            # *recogFixationText_Run2* updates
            if recogFixationText_Run2.status == NOT_STARTED and tThisFlip >= 3.0-frameTolerance:
                # keep track of start time/frame for later
                recogFixationText_Run2.frameNStart = frameN  # exact frame index
                recogFixationText_Run2.tStart = t  # local t and not account for scr refresh
                recogFixationText_Run2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(recogFixationText_Run2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                Fixation_Started_Time = recognitionImageRun2_stopped_time # Fixation onset depends on Image offset
                thisExp.addData('recogFixationText_Run2.started', Fixation_Started_Time) # recogFixationText_Run2.started for Block 2 Run 2  
                recogFixationText_Run2.setAutoDraw(True)
            if recogFixationText_Run2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > recogFixationText_Run2.tStartRefresh + ISI_Time-frameTolerance: # Replaced 9999.0 with ISI_Time for Block 2 Run 2 Recognition
                    # keep track of stop time/frame for later
                    recogFixationText_Run2.tStop = t  # not accounting for scr refresh
                    recogFixationText_Run2.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    fixation_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('recogFixationText_Run2.stopped', fixation_stopped_time) # recogFixationText_Run2.stopped for Block 2 Run 2
                    next_Instruction_started_time = fixation_stopped_time  # Set onset of next instruction time 
                    recogFixationText_Run2.setAutoDraw(False)
                else: # If fixation_stopped_time not updated
                    fixation_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('recogFixationText_Run2.stopped', fixation_stopped_time) # recogFixationText_Run2.stopped for Block 2 Run 2
                    next_Instruction_started_time = fixation_stopped_time  # Set onset of next instruction time
            ...
            if recogFixationText_Run2.status == STARTED:  # only update if drawing
                recogFixationText_Run2.setText(ISI_Message, log=False)
            ...
            if recogTrialTestKeyRespRun2.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                ...
                thisExp.addData('recogTrialTestKeyRespRun2.started', recogTrialTestKeyRespRun2_started_time) # recogTrialTestKeyRespRun2.started For Block 2 Run 2
            ...
            if recogTrialTestKeyRespRun2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > recogTrialTestKeyRespRun2.tStartRefresh + 6-frameTolerance:
                    # keep track of stop time/frame for later
                    ...
                    # add timestamp to datafile
                    recogTrialTestKeyRespRun2_stopped_time = Time_Since_Run.getTime()
                ...
                else: # If recogTrialTestKeyRespRun2_stopped_time not updated
                    recogTrialTestKeyRespRun2_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('recogTrialTestKeyRespRun2.stopped', recogTrialTestKeyRespRun2_stopped_time)  # recogTrialTestKeyRespRun2.stopped for Block 2 Run 2
            ...
            if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
                ...
                # Edit lists for txt at recognitionTrial_Run2 recognition block 2 run 2 upon 'esc'
                handle_esc_experiment_data( 
                recog_key=Recog_Key_Block_2_Run_2,
                block_num=2,
                run_num=Block_2_Run,
                recog_face_stims=Face_Recog_Images_Block_2_Run_2,
                recog_place_stims=Place_Recog_Images_Block_2_Run_2,
                recog_pair_stims=Pair_Recog_Images_Block_2_Run_2,
                displayed_images_face=recog_face_displayed_images_block_2_run_2,
                displayed_images_place=recog_place_displayed_images_block_2_run_2,
                displayed_images_pair=recog_pair_displayed_images_block_2_run_2,
                encoding_response_data_face=Encoding_Response_Data_Face_Block_2_Run_2,
                encoding_response_data_place=Encoding_Response_Data_Place_Block_2_Run_2,
                encoding_response_data_pair=Encoding_Response_Data_Pair_Block_2_Run_2
            )
                ...
                # Update recognitionTrial_Run2 at recognition  block 2 run 2 upon 'esc'
                save_experiment_data(
                expInfo, 
                block_num=Block_2_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_2_Run_2, 
        ...
        if Recog_Key_Block_2_Run_2 == 'Face_Recog_Key_Block_2_Run_2':
            ...
            recog_face_displayed_images_block_2_run_2.append(current_displayed_image)
            ...
            recog_place_displayed_images_block_2_run_2.append(current_displayed_image)
            ...
            recog_pair_displayed_images_block_2_run_2.append(current_displayed_image)
        ...
        if routineForceEnded:
            ...
            routineTimer.addTime(-(3.0 + ISI_Time)) # Replaced -10002 with 3.0 + ISI_Time for Block 2 Run 2 Recognition
        thisExp.nextEntry()
...
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
frameN = -1
Time_Since_Run.reset()  # Reset clock for run 3 onset and stop times
next_Instruction_started_time = 0  # Initialize instruction start time, run 3 onset times depends on this
image_counter = 0 # For instuction onset times i.e., 1st image means Instruction time based on Time_Since_Run
...
for thisBlock3Loop in Block3Loop:
    ...
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        if InstructionText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            Instruction_start = next_Instruction_started_time if next_Instruction_started_time != 0 else Time_Since_Run.getTime()
            thisExp.addData('InstructionText.started', Instruction_start)  # InstructionText.started for Block 3 Run 3 Encoding
            InstructionText.setAutoDraw(True)
        if InstructionText.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > InstructionText.tStartRefresh + 10-frameTolerance:
                ...
                # add timestamp to datafile
                Instruction_stop = Time_Since_Run.getTime()
                thisExp.addData('InstructionText.stopped', Instruction_stop)  # InstructionText.stopped for Block 3 Run 3 Encoding
                InstructionText.setAutoDraw(False)
            else: #  If Instruction_stop not updated
                Instruction_stop = Time_Since_Run.getTime()
                thisExp.addData('InstructionText.stopped', Instruction_stop)  # InstructionText.stopped for Block 3 Run 3 Encoding
        ...
        if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
            ...
            # Update InstructionsText at encoding block 3 run 3 upon 'esc'
            save_experiment_data(
                expInfo, 
                block_num=Block_3_Run, 
                ...
                Recog_Data_Face=Recog_Data_Face_Block_3_Run_3, 
                Recog_Data_Place=Recog_Data_Place_Block_3_Run_3, 
                Recog_Data_Pair=Recog_Data_Pair_Block_3_Run_3,
                ...
                Recog_Response_Data_Face=Recog_Response_Data_Face_Block_3_Run_3, 
                Recog_Response_Data_Place=Recog_Response_Data_Place_Block_3_Run_3, 
                Recog_Response_Data_Pair=Recog_Response_Data_Pair_Block_3_Run_3
            )
    ...
    for thisEncodeLoop_Block3Run3 in encodeLoop_Block3Run3:
        ...
        # keep track of which components have finished
        encodeTrial_Run3Components = [encodeTrialRun3_Image, encodeFixationText_Run3, encodeTrialTestKeyRespRun3]
        ...
        while continueRoutine and routineTimer.getTime() < 3.0 + ISI_Time: # Replaced 10002 with 3.0 + ISI_Time for Block 3 Run 3 Encoding
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if encodeTrialRun3_Image.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                ...
                # add timestamp to datafile
                encodeImageRun3_started_time = Instruction_stop if image_counter == 0 else fixation_stopped_time
                thisExp.addData('encodeTrialRun3_Image.started', encodeImageRun3_started_time)  # encodeTrialRun3_Image for Block 3 Run 3
            ...
            if encodeTrialRun3_Image.status == STARTED:
                ...
                if tThisFlipGlobal > encodeTrialRun3_Image.tStartRefresh + 3-frameTolerance:
                    ...
                    thisExp.addData('encodeTrialRun3_Image.stopped', encodeImageRun3_stopped_time) # encodeTrialRun3_Image for Block 3 Run 3
                    encodeTrialRun3_Image.setAutoDraw(False)
                else:  #  If encodeImageRun3_stopped_time not updated
                    encodeImageRun3_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('encodeTrialRun3_Image.stopped', encodeImageRun3_stopped_time)
            ...
            # *encodeFixationText_Run3* updates
            if encodeFixationText_Run3.status == NOT_STARTED and tThisFlip >= 3.0-frameTolerance:
                # keep track of start time/frame for later
                encodeFixationText_Run3.frameNStart = frameN  # exact frame index
                encodeFixationText_Run3.tStart = t  # local t and not account for scr refresh
                encodeFixationText_Run3.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(encodeFixationText_Run3, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                Fixation_Started_Time = encodeImageRun3_stopped_time # Fixation onset depends on Image offset
                thisExp.addData('encodeFixationText_Run3.started', Fixation_Started_Time) # encodeFixationText_Run3.started for Block 3 Run 3 Encoding
                encodeFixationText_Run3.setAutoDraw(True)
            if encodeFixationText_Run3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > encodeFixationText_Run3.tStartRefresh + ISI_Time-frameTolerance: # Replaced 9999.0 with ISI_Time for Block 3 Run 3 Encoding
                    # keep track of stop time/frame for later
                    encodeFixationText_Run3.tStop = t  # not accounting for scr refresh
                    encodeFixationText_Run3.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    fixation_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('encodeFixationText_Run3.stopped', fixation_stopped_time)  # encodeFixationText_Run3.stopped for Block 3 Run 3 Encoding
                    next_Instruction_started_time = fixation_stopped_time  # Set onset of next instruction time
                    encodeFixationText_Run3.setAutoDraw(False)
                else: # If fixation_stopped_time not updated
                    fixation_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('encodeFixationText_Run3.stopped', fixation_stopped_time) 
                    next_Instruction_started_time = fixation_stopped_time  # Set onset of next instruction time
                    
            if encodeFixationText_Run3.status == STARTED:  # only update if drawing
                encodeFixationText_Run3.setText(ISI_Message, log=False)
            ...
            if encodeTrialTestKeyRespRun3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > encodeTrialTestKeyRespRun3.tStartRefresh + 6-frameTolerance:
                    # keep track of stop time/frame for later
                    ...
                    # add timestamp to datafile
                    encodeTrialTestKeyRespRun3_stopped_time = Time_Since_Run.getTime()
            ...
            if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
                ...
                # Update encodeTrial_Run3 at encoding block 3 run 3 upon 'esc'
                save_experiment_data(
                expInfo, 
                block_num=Block_3_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_3_Run_3, 
        ...
        if current_encode_index == 1: # First stimulus is shown
            Fix_Cross_Started = True # Next ISI is '+'
            encode_trials_counter += 1
            image_counter += 1 # Indicate that first image has been shown
        ...
        if current_encode_index == 12: # Last stimulus is shown
            ...
            current_encode_key_index += 1 # Next encode condition
            current_jitter_index = 0 # Reset current jitter index
            image_counter = 0 # Reset image_counter for encoding condition
        ...
        if routineForceEnded:
            ...
            routineTimer.addTime(-(3.0 + ISI_Time)) # Replaced -10002 with 3.0 + ISI_Time for Block 3 Run 3 Encoding
        thisExp.nextEntry()
    ...
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        if InstructionText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            Instruction_start = next_Instruction_started_time if next_Instruction_started_time != 0 else Time_Since_Run.getTime()
            thisExp.addData('InstructionText.started', Instruction_start)  # InstructionText.started for Block 3 Run 3 Recognition
            InstructionText.setAutoDraw(True)
        if InstructionText.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > InstructionText.tStartRefresh + 10-frameTolerance:
                ...
                # add timestamp to datafile
                Instruction_stop = Time_Since_Run.getTime()
                thisExp.addData('InstructionText.stopped', Instruction_stop)  # InstructionText.stopped for Block 3 Run 3 Recognition
                InstructionText.setAutoDraw(False)
            else: # If Instruction_stop not updated
                Instruction_stop = Time_Since_Run.getTime()
                thisExp.addData('InstructionText.stopped', Instruction_stop)  # InstructionText.stopped for Block 3 Run 3 Recognition
        ...
        if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
            ...
            # Update InstructionsText at recognition block 3 run 3 upon 'esc'
            save_experiment_data(
                expInfo, 
                block_num=Block_3_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_3_Run_3, 
    ...
    for thisRecognitionLoop_Block3Run3 in recognitionLoop_Block3Run3:
        ...
        # keep track of which components have finished
        recognitionTrial_Run3Components = [recogTrialRun3_Image, recogFixationText_Run3, recogTrialTestKeyRespRun3]
        ...
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        frameN = -1
        next_Instruction_started_time = 0 # Reset following every condition - Block 3 Run 3 Recognition
        
        # --- Run Routine "recognitionTrial_Run3" ---
        while continueRoutine and routineTimer.getTime() < 3.0 + ISI_Time: # Replaced 10002 with 3.0 + ISI_Time for Block 3 Run 3 Recognition
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if recogTrialRun3_Image.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                ...
                # add timestamp to datafile
                recognitionImageRun3_started_time = Instruction_stop if image_counter == 0 else fixation_stopped_time
            ...
            if recogTrialRun3_Image.status == STARTED:
                ...
                if tThisFlipGlobal > recogTrialRun3_Image.tStartRefresh + 3-frameTolerance:
                    ...
                    recognitionImageRun3_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('recogTrialRun3_Image.stopped', recognitionImageRun3_stopped_time)
            ...
            # *recogFixationText_Run3* updates
            if recogFixationText_Run3.status == NOT_STARTED and tThisFlip >= 3.0-frameTolerance:
                # keep track of start time/frame for later
                recogFixationText_Run3.frameNStart = frameN  # exact frame index
                recogFixationText_Run3.tStart = t  # local t and not account for scr refresh
                recogFixationText_Run3.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(recogFixationText_Run3, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                Fixation_Started_Time = recognitionImageRun3_stopped_time # Fixation onset depends on Image offset
                thisExp.addData('recogFixationText_Run3.started', Fixation_Started_Time) # recogFixationText_Run3.started for Block 3 Run 3
                recogFixationText_Run3.setAutoDraw(True)
            if recogFixationText_Run3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > recogFixationText_Run3.tStartRefresh + ISI_Time-frameTolerance: # Replaced 9999.0 with ISI_Time for Block 3 Run 3 Recognition
                    # keep track of stop time/frame for later
                    recogFixationText_Run3.tStop = t  # not accounting for scr refresh
                    recogFixationText_Run3.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    fixation_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('recogFixationText_Run3.stopped', fixation_stopped_time) # recogFixationText_Run3.stopped for Block 3 Run 3
                    next_Instruction_started_time = fixation_stopped_time  # Set onset of next instruction time
                    recogFixationText_Run3.setAutoDraw(False)
                else: # If fixation_stopped_time not updated
                    fixation_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('recogFixationText_Run3.stopped', fixation_stopped_time)
                    next_Instruction_started_time = fixation_stopped_time
                    
            if recogFixationText_Run3.status == STARTED:  # only update if drawing
                recogFixationText_Run3.setText(ISI_Message, log=False)
            ...
            if recogTrialTestKeyRespRun3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > recogTrialTestKeyRespRun3.tStartRefresh + 6-frameTolerance:
                    # keep track of stop time/frame for later
                    ...
                    # add timestamp to datafile
                    recogTrialTestKeyRespRun3_stopped_time = Time_Since_Run.getTime()
                    thisExp.addData('recogTrialTestKeyRespRun3.stopped', recogTrialTestKeyRespRun3_stopped_time) # ecogTrialTestKeyRespRun3.stopped for Block 3 Run 
            ...
            if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
                ...
                # Edit lists for txt at recognitionTrial_Run2 recognition block 3 run 3 upon 'esc'
                handle_esc_experiment_data( 
                recog_key=Recog_Key_Block_3_Run_3,
                block_num=3,
                run_num=Block_3_Run,
                recog_face_stims=Face_Recog_Images_Block_3_Run_3,
                recog_place_stims=Place_Recog_Images_Block_3_Run_3,
                recog_pair_stims=Pair_Recog_Images_Block_3_Run_3,
                displayed_images_face=recog_face_displayed_images_block_3_run_3,
                displayed_images_place=recog_place_displayed_images_block_3_run_3,
                displayed_images_pair=recog_pair_displayed_images_block_3_run_3,
                encoding_response_data_face=Encoding_Response_Data_Face_Block_3_Run_3,
                encoding_response_data_place=Encoding_Response_Data_Place_Block_3_Run_3,
                encoding_response_data_pair=Encoding_Response_Data_Pair_Block_3_Run_3
            )
                ...
                # Update recognitionTrial_Run3 at recognition  block 3 run 3 upon 'esc'
                save_experiment_data(
                expInfo, 
                block_num=Block_3_Run, 
                Encoding_Data_Face=Encoding_Data_Face_Block_3_Run_3, 
        ...
        if Recog_Key_Block_3_Run_3 == 'Face_Recog_Key_Block_3_Run_3':
            ...
            recog_face_displayed_images_block_3_run_3.append(current_displayed_image)
            ...
            recog_place_displayed_images_block_3_run_3.append(current_displayed_image)
            ...
            recog_pair_displayed_images_block_3_run_3.append(current_displayed_image)
        ...
        if routineForceEnded:
            ...
            routineTimer.addTime(-(3.0 + ISI_Time)) # Replaced -10002 with 3.0 + ISI_Time for Block 3 Run 3 Recognition
        thisExp.nextEntry()
